...
IMAGENES=(NoCountryForOldMen.bmp Wargames.bmp BigFish.bmp)
...
for imp in asm; do
  ...
  for s in ${SIZESMEM[*]}; do
    run_test "$TP2ALU" "Bordes" "$imp" "$TESTINDIR/$img1.$s.bmp" ""
  ...
  for s in ${SIZESMEM[*]}; do
    v=0.555
    run_test "$TP2ALU" "Rombos" "$imp" "$TESTINDIR/$img1.$s.bmp" ""
    if [ $ret -ne 0 ]; then exit -1; fi
  done

  # Nivel
  for s in ${SIZESMEM[*]}; do
    run_test "$TP2ALU" "Nivel" "$imp" "$TESTINDIR/$img1.$s.bmp" "5"
    if [ $ret -ne 0 ]; then exit -1; fi
  done
